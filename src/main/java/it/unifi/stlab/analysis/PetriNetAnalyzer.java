    ...
    public static Map<String, String> createExoFaultsMap(List<Component> components, List<String> failureNames) {
        ...
        for (Component component : components) {
            ...
            for (ErrorMode errorMode : component.getErrorModes()) {
                String failureName = errorMode.getOutgoingFailure().getDescription().trim() + "_" + name;
                failureNames.add(failureName);
            }

            List<PropagationPort> propagationPorts = component.getPropagationPorts();

            for (PropagationPort propagationPort : propagationPorts) {
                String failureName = propagationPort.getPropagatedFailureMode().getDescription().trim() + "_" + name;
                String exoFaultName = propagationPort.getExogenousFaultMode().getName().trim() + "_" + name;
