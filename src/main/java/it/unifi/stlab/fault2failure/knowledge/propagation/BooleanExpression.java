    ...
    static BooleanExpression config(String booleanExpression, HashMap<String, FailureMode> failModes) {
        ...
        if (booleanExpression.contains("&&")) {
            ...
            for (String literal : booleanExpression.split("&&")) {
                if (literal.contains("&&"))
                    be.addChild(config(literal, failModes));
                else if (literal.contains("OR"))
                    be.addChild(config(literal, failModes));
                else if (literal.contains(("!"))) {
                    be.addChild(config(literal, failModes));
                } else {
                    if (failModes.containsKey(literal.trim()))
                        be.addChild(failModes.get(literal.trim()));
                    else {
                        FailureMode temp = new FailureMode(literal.trim().replaceAll("[\\[\\](){}]", ""));
                        be.addChild(temp);
                        failModes.put(literal.trim(), temp);
                    }
                }
            }
        }
            ...
            be = new NOT();
            String literal = booleanExpression.replaceFirst("!", "");
            if (literal.contains("&&"))
                be.addChild(config(literal, failModes));
            else if (literal.contains("OR"))
                be.addChild(config(literal, failModes));
            ...
            be = new OR();
            for (String literal : booleanExpression.split("OR")) {
                if (literal.contains("&&"))
                    be.addChild(config(literal, failModes));
                else if (literal.contains("OR"))
                    be.addChild(config(literal, failModes));
